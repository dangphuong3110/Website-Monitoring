...
class Tool
{
    ...
    public static function dayStringFromTimestamp($timestamp)
    {
        if (isset($timestamp) && $timestamp != '0000-00-00 00:00:00') {
            ...
            $result = \App\Library\Tool::dateTime($timestamp)->format('Y-m-d');
        } else {
            $result = \App\Library\Tool::dateTime(\Carbon\Carbon::now())->format('Y-m-d');
        }
    ...
    public static function timeStringFromTimestamp($timestamp)
    {
        if (isset($timestamp) && $timestamp != '0000-00-00 00:00:00') {
            ...
            $result = \App\Library\Tool::dateTime($timestamp)->format('H:i');
        } else {
            $result = \App\Library\Tool::dateTime(\Carbon\Carbon::now())->format('H:i');
        }
     ...
     * @return string
     */
    public static function cronjobUpdateController($req, $controller)
    {
        ...
        $setting_php_bin_path = \App\Models\Setting::get('php_bin_path');
        ...
        if (!empty($req->old())) {
            $php_bin_path = $req->old()['php_bin_path'];
            $php_bin_path_value = $req->old()['php_bin_path_value'];
            $queue_driver = $req->old()['queue_driver'];
        }

        // create remote token if empty
        if (empty(\App\Models\Setting::get('remote_job_token'))) {
            \App\Models\Setting::set('remote_job_token', str_random(60));
        }

        $req->session()->forget('cron_jobs');
        $error = '';
        $valid = false;
        if ($req->isMethod('post')) {
            $php_bin_path = $req->php_bin_path;
            $php_bin_path_value = $req->php_bin_path_value;
            $queue_driver = $req->queue_driver;

            // If type == database
            if ($req->queue_driver == 'database') {
                $rules = [
                ...
                // Check valid path
                $check = \App\Library\Tool::checkPHPBinPath($php_bin_path_value);
                if ($check != 'ok') {
                    $rules['php_bin_path_invalid'] = 'required';
                }

                $controller->validate($req, $rules);

                \App\Models\Setting::set('php_bin_path', $php_bin_path_value);

                $valid = true;
            }

            $req->session()->put('cron_jobs', true);

            // Update .env
            if (in_array($queue_driver, ['database', 'async']) && config('queue.default') != $queue_driver) {
                \App\Models\Setting::setEnv('QUEUE_DRIVER', $queue_driver);
            }

            if ($req->queue_driver == 'async') {
                return 'done';
            }

            $req->session()->flash('alert-success', trans('messages.setting.updated'));
        }
     ...
     * @return bool
     */
    public static function checkReCaptcha($req)
    {
        if (!isset($req->all()['g-recaptcha-response'])) {
            return false;
        }
        ...
        $res = $client->post('https://www.google.com/recaptcha/api/siteverify', ['verify' => false, 'form_params' => [
            ...
            'remoteip' => $req->ip(),
            'response' => $req->all()['g-recaptcha-response'],
        ]]);
